...
class Connection():
    ...
    进行JSON文件的输出以供前端调用"""
    def __init__(self,address,username,passwd,retry=0):
        """初始化连接过程，尝试连接数据库"""
        code = 0
        for x in range(retry+1):
            code = self.connectDB(address,username,passwd,retry)
            if code == 1:break
            sleep(3)
        if code != 1 : raise ConnectionError('Connection Failed')
                
    def connectDB(self,address,username,passwd,retry=False):
        """连接数据库的主要方法"""
        self.connect = pymongo.MongoClient(str(address))
        code,self.logininfo,self.status = self.getDBInfo()
        if code == 1:
            self.db = self.connect.alidb
...
if __name__ == "__main__":
    ...
    try:
        print("\n\n\n","=="*60,"\n",time.ctime())
        con = Connection("file.mazhangjing.com","","",retry=3)
        con.getStatus()
        lab = con.db.lab
